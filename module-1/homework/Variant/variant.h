...
constexpr size_t CheckDuplicates(size_t cur_pos, const bool (&exact)[SizeofFound],
    ...
    return CheckDuplicatesHelper(cur_pos, CheckDuplicates(cur_pos + 1, exact, convertible), exact, convertible);
}

template<typename TargetType, typename... Types>
struct FindExactlyOneChecked {
    constexpr static bool kFoundExact[sizeof...(Types)] = {std::is_same<TargetType, Types>::value...};
    constexpr static bool kFoundConvertible[sizeof...(Types)] = {std::is_convertible<TargetType, Types>::value...};
    constexpr static size_t kValue = CheckDuplicates(0, kFoundExact, kFoundConvertible);

    static_assert(kValue != kNotFound, "no such type in parameter list");
    static_assert(kValue != kAmbiguity, "there are several occurrences of the same type in parameter list");
};
